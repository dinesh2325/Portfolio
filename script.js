...
hamburger?.addEventListener("click", () => {
  ...
  bars.forEach((bar, index) => {
    bar.style.transform = hamburger.classList.contains("active")
      ? `rotate(${index === 0 ? 45 : index === 1 ? 0 : -45}deg) translate(${
          index === 0 ? 6 : index === 1 ? 20 : 6
        }px, ${index === 0 ? 6 : index === 1 ? 0 : -6}px)`
      : "none";
    bar.style.opacity =
      hamburger.classList.contains("active") && index === 1 ? "0" : "1";
  });
});
...
function handleParallax() {
  ...
  const parallaxElements = document.querySelectorAll(
    ".hero::before, .floating-particles"
  );
...
function initScrollAnimations() {
  const animateElements = document.querySelectorAll(
    ".reveal-text, .animated-stat, .project-card, .achievement-card"
  );
...
function enhanceParticles() {
  ...
  particles.forEach((particle, index) => {
    ...
    particle.addEventListener("mouseleave", () => {
      particle.style.transform = particle.style.transform.replace(
        " scale(1.5)",
        ""
      );
...
function activateEasterEgg() {
  showNotification(
    "🎉 Konami Code activated! Secret developer mode enabled!",
    "success"
  );
    ...
    card.addEventListener("mouseenter", () => {
      ...
      if (medal) {
        medal.style.boxShadow =
          "0 0 40px rgba(255, 215, 0, 0.8), 0 0 60px rgba(102, 126, 234, 0.4)";
...
function addInteractiveBackground() {
  const backgrounds = document.querySelectorAll(
    ".experience::before, .skills::before"
  );
...
function enhanceResponsiveExperience() {
  // Optimize for mobile devices
  if (window.innerWidth <= 768) {
    // Disable animations on very small devices for better performance
    if (window.innerWidth <= 480) {
      document
        .querySelectorAll(".floating-particles, .particle, .tech-icon")
        .forEach((el) => {
...
// Handle viewport resize for responsive optimizations
function handleViewportResize() {
  ...
  window.addEventListener("resize", () => {
    ...
    resizeTimer = setTimeout(() => {
      ...
      // Update navbar background based on current scroll position
      if (scrollY > 100) {
        navbar.style.background = "rgba(10, 10, 10, 0.98)";
        navbar.style.backdropFilter = "blur(20px)";
      } else {
      ...
      // Reflow layout for complex components
      const experienceCards = document.querySelectorAll(".experience-card");
      experienceCards.forEach((card) => {
...
// Enhanced loading screen with responsive features
function enhanceLoadingScreen() {
  ...
  if (loadingScreen) {
    ...
    if (window.innerWidth <= 768) {
      ...
      const loadingLogo = loadingScreen.querySelector(
        ".loading-logo .logo-text"
      );
      if (loadingLogo) {
        loadingLogo.style.fontSize = "2.5rem";
      }
    }
  }
}
...
function initResponsiveEnhancements() {
  ...
  window.addEventListener(
    ...
    throttle(() => {
      ...
      document.body.classList.toggle("small-mobile", window.innerWidth <= 480);
    }, 250)
  );
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Ensure About section text stays visible permanently
  ensureAboutTextVisible();

  // Add easter egg click count
  let clickCount = 0;
  ...
  // Add mouse trail only on desktop
  if (window.innerWidth > 768) {
...
// Function to ensure About section text stays visible
function ensureAboutTextVisible() {
  const revealTexts = document.querySelectorAll(".reveal-text");

  // Set immediate fallback visibility
  ...
  revealTexts.forEach((text, index) => {
    ...
    // Fallback timer to ensure visibility after 12 seconds
    setTimeout(() => {
      text.style.opacity = "1";
      text.style.transform = "translateY(0)";
      text.style.visibility = "visible";
